...
El dataset tiene `r d[1]` observaciones con `r d[2]` variables.  Vemos que cada parada está duplicada por cada sentido y línea por lo que será necesario ordenar los datos para poder construir nuestro grafo con la librería `igraph`
...
Con esto ya podemos crear una lista de líneas donde cada linea es una tabla con la información relevante para el grafo. Nos quedamos solo con el sentido 1 y elegimos las variables `NUMEROORDEN`, `DENOMINACION`, `CODIGOESTACION`,`LONGITUDTRAMOANTERIOR` y `VELOCIDADTRAMOANTERIOR`. Añadimos manualmente la estación de origen como la primera y las ordenamos.
...
for (x in cabezaTermino$LINEA) {
  ...
  if (!(nombre_linea %in% c("12-2","6-2"))){
    lineas[[nombre_linea]] <- datos %>%
      ...
      select(NUMEROORDEN, DENOMINACION, CODIGOESTACION, LONGITUDTRAMOANTERIOR, VELOCIDADTRAMOANTERIOR) %>% mutate(TIEMPOTRAMOANTERIOR = 60*(LONGITUDTRAMOANTERIOR/(1000*VELOCIDADTRAMOANTERIOR))) %>%
      add_row(
        NUMEROORDEN = 1, 
